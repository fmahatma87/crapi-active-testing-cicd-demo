...
export function* forgotPassword(param) {
  ...
  try {
    yield put({ type: actionTypes.FETCHING_DATA })
    const postUrl = APIService.JAVA_MICRO_SERVICES + requestURLS.FORGOT_PASSWORD
    const headers = {
      'Content-Type': 'application/json',
    }
...
export function* verifyOTP(param) {
  ...
  try {
    ...
    else if (recievedResponse.status === 503) callback(responseTypes.REDIRECT, responseJSON.message)
    else callback(responseTypes.FAILURE, responseJSON.message)
  } catch (e) {
    yield put({ type: actionTypes.FETCHED_DATA, payload: recievedResponse })
...
export function* getServices(param) {
  ...
  try {
    yield put({ type: actionTypes.FETCHING_DATA })
    const getUrl = APIService.PYTHON_MICRO_SERVICES + requestURLS.GET_SERVICES
    const headers = {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${accessToken}`,
    }
